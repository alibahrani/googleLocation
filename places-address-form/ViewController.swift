...
class ViewController: UIViewController {
  ...
  @IBAction func autocompleteClicked(_ sender: UIButton) {
    ...
    addressFilter.type = .establishment
...
extension ViewController: GMSAutocompleteViewControllerDelegate {
  ...
  func viewController(_ viewController: GMSAutocompleteViewController, didFailAutocompleteWithError error: Error) {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "contactSaved"{
            let controller = segue.destination as! SaveContactViewController
            controller.selectedAddress  = address_line_1.text!
            controller.selectedPostcode = postal_code_field.text!
            controller.selectedCountry = country_field.text!
            controller.selectedState = state.text!
